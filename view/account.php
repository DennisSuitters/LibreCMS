...
if($user['rank']>0){
    $content.='<div id="libr8_admin_header">';
		$content.='<nav class="libr8-navbar libr8-navbar-inverse" role="navigation">';
	...
	$content.='</div>';
	$content.='<div class="libr8-collapse libr8-navbar-collapse" id="libr8-admin-collapse">';
		$content.='<ul class="libr8-nav libr8-navbar-nav libr8-pull-right libr8-relative" style="margin-right:20px;">';
    if($user['rank']>899){
        if($user['options']{3}==1){
	   ...
	   if($user['options']{0}==1){
			$content.='<li>';
				$content.='<button class="libr8-btn libr8-btn-success libr8-dropdown-toggle" style="margin:7px 10px;" data-toggle="dropdown" data-placement="right">Add <i class="libr8-caret"></i></button>';
				$content.='<ul class="libr8-dropdown-menu libr8-multi-level libr8-pull-right">';
           ...
           while($sr=$sq->fetch(PDO::FETCH_ASSOC)){
					...
					$content.='<li class="libr8-dropdown-submenu libr8-pull-left">';
    ...
    $content.='<img id="avatar" class="libr8-pull-right libr8-img-circle" src="'.$avatar.'"></div></div></nav></div>';
}
