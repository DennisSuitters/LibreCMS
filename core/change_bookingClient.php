...
if($id==0)
  $c=array('id'=>0,'username'=>'','name'=>'','business'=>'','address'=>'','suburb'=>'','state'=>'','city'=>'','postcode'=>'','email'=>'','phone'=>'','mobile'=>'');
else{
  $q=$db->prepare("SELECT * FROM login WHERE id=:id");$q->execute(array(':id'=>$id));$c=$q->fetch(PDO::FETCH_ASSOC);
}
...
$q->execute(array(':cid'=>$id,':business'=>$c['business'],':name'=>$c['name'],':address'=>$c['address'],':suburb'=>$c['suburb'],':state'=>$c['state'],':city'=>$c['city'],':postcode'=>$c['postcode'],':email'=>$c['email'],':phone'=>$c['phone'],':mobile'=>$c['mobile'],':id'=>$bid));?>
