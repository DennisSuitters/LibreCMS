...
if ($show == 'categories') {
	...
	if (stristr($html,'<print page=cover>')){
		if ($page['cover'] != '' || $page['coverURL'] !='') {
			...
			$html = str_replace('<print page=cover>', $coverLink, $html);
		} else 
	...
	if ($config['business'])
		$html = preg_replace('/<print content=[\"\']?seoTitle[\"\']?>/', htmlspecialchars($config['business'], ENT_QUOTES, 'UTF-8'), $html);
	else
		$html = preg_replace('/<print content=[\"\']?seoTitle[\"\']?>/', htmlspecialchars($config['seoTitle'], ENT_QUOTES, 'UTF-8'), $html);
