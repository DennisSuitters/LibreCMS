...
if($ii>0){
	...
	foreach($featuredfiles as$key=>$r){
		...
		if($r['contentType']=='carousel')$item=preg_replace('~<caption>.*?<\/caption>~is',$r['seoCaption'],$item,1);
			...
			if($r['seoCaption']!='')$item=preg_replace('/<print content=[\"\']?caption[\"\']?>/',$r['seoCaption'],$item);
			elseif($r['notes']!='')$item=preg_replace('/<print content=[\"\']?caption[\"\']?>/',strip_tags(rawurldecode($r['notes'])),$item);
