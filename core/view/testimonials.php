...
$item=$matches[1];
if($count==0)
	$s=$db->query("SELECT * FROM content WHERE contentType='testimonials' ORDER BY ti DESC");
else
	$s=$db->query("SELECT * FROM content WHERE contentType='testimonials' ORDER BY ti DESC LIMIT $count");
$i=0;
$items='';
$testitems='';
if($s->rowCount()>0){
