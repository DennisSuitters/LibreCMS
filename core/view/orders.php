...
if($_SESSION['loggedin']==false)$html=$theme['settings']['page_not_found'];
    ...
    if(stristr($html,'<items')){
        ...
        while($r=$s->fetch(PDO::FETCH_ASSOC)){
            $item=$items;
            $item=str_replace('<print orders=ordernumber>',$r['qid'].$r['iid'],$item);
            $item=str_replace('<print orders=status>',$r['status'],$item);
            if($r['iid_ti']>0)
    ...
    if(isset($args[0])&&$args[0]!=''){
        ...
        if($s->rowCount()>0){
            ...
            $order=str_replace('<print order=ordernumber>',$r['qid'].$r['iid'],$order);
            if($r['iid_ti']>0)
                $order=str_replace('<print order=date>',date($config['dateFormat'],$r['iid_ti']),$order);
            else
                $order=str_replace('<print order=date>',date($config['dateFormat'],$r['qid_ti']),$order);
