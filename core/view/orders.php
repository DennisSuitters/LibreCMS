...
if ($_SESSION['loggedin'] == false)
  ...
  if (stristr($html, '<items>')) {
    ...
    while ($r = $s -> fetch(PDO::FETCH_ASSOC)) {
      ...
      if ($r['iid_ti'] > 0)
        $item = preg_replace('/<print order=[\"\']?date[\"\']?>/', date($config['dateFormat'], $r['iid_ti']), $item);
      else
        $item = preg_replace('/<print order=[\"\']?date[\"\']?>/', date($config['dateFormat'], $r['qid_ti']), $item);
      $item = preg_replace(
        array(
          '/<print order=[\"\']?duedate[\"\']?>/',
          '/<print link>/'
  ...
  if (isset($args[0]) && $args[0] != '') {
    ...
    if ($s -> rowCount() > 0) {
      ...
      if ($r['iid_ti'] > 0)
        $order = preg_replace('/<print order=[\"\']?date[\"\']?>/', date($config['dateFormat'], $r['iid_ti']), $order);
      else
        $order = preg_replace('/<print order=[\"\']?date[\"\']?>/', date($config['dateFormat'], $r['qid_ti']), $order);
      $order = preg_replace(
        array(
          '/<print order=[\"\']?notes[\"\']?>/',
