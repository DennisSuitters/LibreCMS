...
if($act=='add_booking'){
			...
			if(is_null($e[2])){
				if($config['email']!=''){
					...
					if($config['bookingAutoReplyLayout']){
						...
						$bookingService;
						if($tis!=0)
							$bookingDate=date($config['dateFormat'],$tis);
						if($rid!=0)
							$bookingService=ucfirst(rtrim($r['contentType'],'s')).' - '.$r['title'];
	...
	if($sql->rowCount()>0){
		...
		while($row=$sql->fetch(PDO::FETCH_ASSOC)){
			...
			if($row['code']!='')
				$bookable.=':'.htmlentities($row['code'],ENT_QUOTES,'UTF-8');
			$bookable.=':'.htmlentities($row['title'],ENT_QUOTES,'UTF-8').'</option>';
		}
		$html=str_replace('<serviceoptions>',$bookable,$html);
		$html=str_replace(array('<bookservices>','</bookservices>'),'',$html);
	}else
