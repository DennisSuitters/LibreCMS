 ...
 */
$content .= '<main id="content" class="col-xs-12 col-md-8"><div class="panel-body">';
...
while ($r = $s -> fetch(PDO::FETCH_ASSOC)) {
	...
	while ($rs = $ss -> fetch(PDO::FETCH_ASSOC)) $content .= '<a href="' . $r['contentType'] . '/' . urlencode(str_replace(' ', '-', $rs['title'])) . '">' . $rs['title'] . '</a><br>';
	$content .= '</div></div>';
}
$content .= '</div></main>';
