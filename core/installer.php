...
<?php }
	...
	if($act=='step3'){
		...
		$sql=$db->prepare("INSERT INTO `".$prefix."login` (options,adminTheme,username,password,email,hash,name,language,ti,active,activate,rank) VALUES ('11111111','default',:username,:password,:email,:hash,:name,'en-AU',:ti,'1','','1000')");
		$sql->execute(
			array(
				':username'=>$aUsername,
				':password'=>$hash,
