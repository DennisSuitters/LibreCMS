...
<?php if($search!=''){
  $qry="SELECT * FROM";
  if($what=='content')$qry.=" `".$prefix."content` WHERE LOWER(seoKeywords) LIKE LOWER(:search) OR LOWER(barcode) LIKE LOWER(:search) OR LOWER(fccid) LIKE LOWER(:search) OR LOWER(code) LIKE LOWER(:search) OR LOWER(brand) LIKE LOWER(:search) OR LOWER(title) LIKE LOWER(:search) OR LOWER(category_1) LIKE LOWER(:search) OR LOWER(category_2) LIKE LOWER(:search) OR LOWER(name) LIKE LOWER(:search) OR LOWER(url) LIKE LOWER(:search) OR LOWER(email) LIKE LOWER(:search) OR LOWER(business) LIKE LOWER(:search) OR LOWER(address) LIKE LOWER(:search) OR LOWER(suburb) LIKE LOWER(:search) OR LOWER(city) LIKE LOWER(:search) OR LOWER(state) LIKE LOWER(:search) OR LOWER(postcode) LIKE LOWER(:search) OR LOWER(phone) LIKE LOWER(:search) OR LOWER(mobile) LIKE LOWER(:search) OR LOWER(attributionImageTitle) LIKE LOWER(:search) OR LOWER(attributionImageName) LIKE LOWER(:search) OR LOWER(exifISO) LIKE LOWER(:search) OR LOWER(exifAperture) LIKE LOWER(:search) OR LOWER(exifFocalLength) LIKE LOWER(:search) OR LOWER(exifShutterSpeed) LIKE LOWER(:search) OR LOWER(exifCamera) LIKE LOWER(:search) OR LOWER(exifLens) LIKE LOWER(:search) OR LOWER(cost) LIKE LOWER(:search) OR LOWER(subject) LIKE LOWER(:search) OR LOWER(notes) LIKE LOWER(:search) OR LOWER(attributionContentName) LIKE LOWER(:search) OR LOWER(tags) LIKE LOWER(:search) OR LOWER(seoCaption) LIKE LOWER(:search) OR LOWER(seoDescription) LIKE LOWER(:search)";
  if($what=='comments')$qry.=" `".$prefix."comments` WHERE LOWER(email) LIKE LOWER(:search) OR LOWER(name) LIKE LOWER(:search) OR LOWER(notes) LIKE LOWER(:search)";
  if($what=='messages')$qry.=" `".$prefix."messages` WHERE LOWER(to_email) LIKE LOWER(:search) OR LOWER(to_name) LIKE LOWER(:search) OR LOWER(from_email) LIKE LOWER(:search) OR LOWER(from_name) LIKE LOWER(:search) OR LOWER(subject) LIKE LOWER(:search) OR LOWER(notes_raw) LIKE LOWER(:search) OR LOWER(attachments) LIKE LOWER(:search)";
  if($what=='orders')$qry.=" `".$prefix."orders` WHERE LOWER(qid) LIKE LOWER(:search) OR LOWER(iid) LIKE LOWER(:search) OR LOWER(did) LIKE LOWER(:search) OR LOWER(aid) LIKE LOWER(:search) OR LOWER(notes) LIKE LOWER(:search)";
  if($what=='pages')$qry.=" `".$prefix."menu` WHERE LOWER(title) LIKE LOWER(:search) OR LOWER(seoTitle) LIKE LOWER(:search) OR LOWER(attributionImageTitle) LIKE LOWER(:search) OR LOWER(attributionImageName) LIKE LOWER(:search) OR LOWER(seoKeywords) LIKE LOWER(:search) OR LOWER(seoDescription) LIKE LOWER(:search) OR LOWER(seoCaption) LIKE LOWER(:search) OR LOWER(notes) LIKE LOWER(:search)";
  ...
  if($ord=='asc')$qry.=$what=='pages'?" ORDER BY title ASC":" ORDER BY ti ASC";
  if($ord=='desc')$qry.=$what=='pages'?" ORDER BY title DESC":" ORDER BY ti DESC";
  $s=$db->prepare($qry);
  ...
  while($r=$s->fetch(PDO::FETCH_ASSOC)){?>
    <div class="searchresults clearfix" data-status="<?php echo$r['status'];?>">
      <ol class="breadcrumb">
        ...
        <li><a href="<?php echo URL.$settings['system']['admin'].'/'.$what.'/edit/'.$r['id'];?>"><?php echo$what=='orders'?$r['qid'].$r['iid'].($r['aid']!=''?'/'.$r['aid']:''):$r['title'];?></a></li>
      </ol>
