...
<div class="page-toolbar">
    <div class="btn-group pull-right">
        <a class="btn btn-success" href="<?php echo URL.$settings['system']['admin'].'/accounts"';if($config['options']{4}==1)echo' data-toggle="tooltip" data-placement="left" title="Back"';?>><i class="libre libre-back"></i></a>
    </div>
</div>
<div class="panel panel-default">
    <div id="covertop">
        <div class="badger badger-left text-shadow-depth-1" data-status="<?php if($r['active']==1)echo'active';else echo'inactive';?>" data-contenttype="<?php echo rank($r['rank']);?>">
        </div>
        <div id="coverimg">
        ...
        <img class="avatar img-thumbnail shadow-depth-1" src="<?php if($r['avatar']!=''&&file_exists('media/avatar/'.$r['avatar']))echo'media/avatar/'.$r['avatar'];elseif($r['gravatar']!=''){if(stristr($r['gravatar'],'@'))echo'http'.md5($r['gravatar']);elseif(stristr($r['gravatar'],'gravatar.com/avatar/'))echo$r['gravatar'];else echo$noavatar;}else echo$noavatar;?>">
    </div>
        ...
        <fieldset class="control-fieldset">
            ...
            <div class="form-group">
                <div class="input-group col-xs-7 col-sm-9 col-lg-10 pull-right">
                    ...
                    <div class="input-group-btn">
                        <a class="btn btn-info" data-toggle="modal" data-target="#media" href="core/browse_media.php?id=<?php echo$r['id'];?>&t=login&c=cover"><span class="libre-stack"><i class="libre libre-stack-1x libre-picture"></i><i class="libre libre-stack-1x libre-action text-info"></i><i class="libre libre-stack-action libre-action-select"></i></span></a>
            ...
            <div class="well col-xs-12 col-sm-10 pull-right">
                ...
                <div class="form-group">
                    ...
                    <div class="input-group col-xs-7 col-sm-9 col-lg-10">
                        <input type="text" id="attributionImageTitle" class="form-control textinput" value="<?php echo$r['attributionImageTitle'];?>" data-dbid="<?php echo$r['id'];?>" data-dbt="login" data-dbc="attributionImageTitle" placeholder="Enter a Title...">
                    </div>
                </div>
                <div class="form-group">
                    <label for="attributionImageName" class="control-label col-xs-5 col-sm-3 col-lg-2">Name</label>
                    <div class="input-group col-xs-7 col-sm-9 col-lg-10">
                        <input type="text" id="attributionImageName" class="form-control textinput" value="<?php echo$r['attributionImageName'];?>" data-dbid="<?php echo$r['id'];?>" data-dbt="login" data-dbc="attributionImageName" placeholder="Enter a Name...">
                    </div>
                </div>
                <div class="form-group">
                    <label for="attributionImageURL" class="control-label col-xs-5 col-sm-3 col-lg-2">URL</label>
                    <div class="input-group col-xs-7 col-sm-9 col-lg-10">
                        <input type="text" id="attributionImageURL" class="form-control textinput" value="<?php echo$r['attributionImageURL'];?>" data-dbid="<?php echo$r['id'];?>" data-dbt="login" data-dbc="attributionImageURL" placeholder="Enter a URL...">
                    </div>
                </div>
            </div>
        </fieldset>
    ...
    <div class="pull-right">
        <div class="btn-group">
            <div class="btn-group" data-toggle="buttons">
                <label class="btn btn-default<?php if($user['layoutAccounts']=='card'){echo' active';}?>"<?php if($config['options']{4}==1)echo' data-toggle="tooltip" data-placement="left" title="Cards"';?>>
                    <input type="radio" name="options" id="option1" autocomplete="off" onchange="update('<?php echo$user['id'];?>','login','layoutAccounts','card');"<?php if($user['layoutAccounts']=='card')echo' checked';?>>
                    <i class="libre libre-layout-blocks"></i>
                </label>
                <label class="btn btn-default<?php if($user['layoutAccounts']=='list')echo' active';?>"<?php if($config['options']{4}==1)echo' data-toggle="tooltip" data-placement="left" title="List"';?>>
                    <input type="radio" name="options" id="option2" autocomplete="off" onchange="update('<?php echo$user['id'];?>','login','layoutAccounts','list');"<?php if($user['layoutAccounts']=='list')echo' checked';?>><i class="libre libre-layout-list"></i>
                </label>
            </div>
        </div>
        <div class="btn-group">
...
<?php if($user['rank']==1000||$user['options']{0}==1){?>
        <div class="btn-group">
            <a class="btn btn-success" href="<?php echo URL.$settings['system']['admin'].'/accounts/add';?>"><i class="libre libre-add"></i></a>
        </div>
<?php }?>
    </div>
</div>
    ...
    <div class="panel-body">
        <div class="table-responsive">
            <table class="table table-condensed table-striped table-hover">
                <thead>
                    <tr>
                        <th class="col-xs-8">Username/Name</th>
...
<?php while($r=$s->fetch(PDO::FETCH_ASSOC)){?>
                    <tr id="l_<?php echo$r['id'];?>" class="item">
                        ...
                        <td><?php echo rank($r['rank']);?></td>
                        <td><?php echo$r['status'];?></td>
                        <td id="controls_<?php echo$r['id'];?>" class="text-right">
                            <a class="btn btn-info btn-sm" href="<?php echo$settings['system']['admin'].'/accounts/edit/'.$r['id'];?>"<?php if($config['options']{4}==1)echo' data-toggle="tooltip" title="Edit"';?>><i class="libre libre-edit"></i></a>
<?php if($user['rank']==1000||$user['options']{0}==1){?>
