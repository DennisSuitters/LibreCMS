...
<?php if($user['rank']==1000||$user['options']{0}==1){?>
      <div class="btn-group"<?php if($config['options']{4}==1)echo' data-toggle="tooltip" data-placement="left" title="Add"';?>>
        <button class="btn btn-default add dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><?php svg('add');?></button>
...
<?php }?>
      <div class="btn-group">
        <a target="_blank" class="btn btn-default info" href="https://github.com/StudioJunkyard/LibreCMS/wiki/Administration#content"<?php if($config['options']{4}==1)echo' data-toggle="tooltip" data-placement="left" title="Help"';?>><?php svg('help');?></a>
      </div>
    </div>
  </div>
...
<?php if($r['contentType']=='article'||$r['contentType']=='events'||$r['contentType']=='news'||$r['contentType']=='proofs'){
  ...
  $cnt=$sc->fetch(PDO::FETCH_ASSOC);?>
              <a class="btn btn-default" href="<?php echo URL.$settings['system']['admin'].'/content/edit/'.$r['id'];?>#d43"<?php if($config['options']{4}==1)echo' data-toggle="tooltip" title="Comments"';?>><?php svg('comments');?> <?php echo$cnt['cnt'];?></a>
<?php }?>
            </td>
            <td class="text-center hidden-xs">
...
if($rr['num']!=0)echo'<a href="'.URL.$settings['system']['admin'].'/content/edit/'.$r['id'].'#d60">'.$rr['num'].'/'.$rr['cnt'].'</a>';?>
            </td>
            <td class="text-center hidden-xs">
              <button class="btn btn-default trash" onclick="$('#views<?php echo$r['id'];?>').text('0');update('<?php echo$r['id'];?>','content','views','0');"><?php svg('eraser');?> <span id="views<?php echo$r['id'];?>"><?php echo$r['views'];?></span></button>
            </td>
            <td id="controls_<?php echo$r['id'];?>" class="text-right">
              <a id="pin<?php echo$r['id'];?>" class="btn btn-default<?php if($r['pin']{0}==1)echo' btn-success';?> hidden-xs" onclick="pinToggle('<?php echo$r['id'];?>','content','pin','0')"<?php if($config['options']{4}==1)echo' data-toggle="tooltip" title="Pin"';?>><?php svg('pin');?></a>
...
<?php if($user['rank']==1000||$user['options']{0}==1){?>
      ...
      <div class="btn-group">
        <a target="_blank" class="btn btn-default info" href="https://github.com/StudioJunkyard/LibreCMS/wiki/Administration#content-edit"<?php if($config['options']{4}==1)echo' data-toggle="tooltip" data-placement="left" title="Help"';?>><?php svg('help');?></a>
      </div>
    </div>
  </div>
<?php }?>
...
elseif($r['fileURL']!='')
  ...
  echo'<img id="fileimage" src="">';?>
            ...
            <small class="help-block pull-right">Using the <span class="btn btn-default btn-xs"><?php svg('magic');?></span> button will resize the uploaded image. (<?php echo$theme['settings']['image_width'].'x'.$theme['settings']['image_height'];?>)</small>
          </div>
...
if($r['thumb']!=''&&file_exists('media'.DS.$rthumb))echo'<a href="'.$r['thumb'].'" data-featherlight="image"><img id="thumbimage" src="'.$r['thumb'].'"></a>';else echo'<img id="thumbimage" src="">';?>
            ...
            <small class="help-block pull-right">Using the <span class="btn btn-default btn-xs"><?php svg('magic');?></span> button will create a Thumbnail from the Center of the above uploaded image. (<?php echo$theme['settings']['thumb_width'].'x'.$theme['settings']['thumb_height'];?>). Uploaded Images take Precedence over URL's.</small>
          </div>
          <fieldset id="d30" class="control-fieldset">
            ...
            <div class="clearfix">
              <small class="help-block col-xs-7 col-sm-9 col-lg-10 pull-right text-right">
                Using the <span class="btn btn-default btn-xs"><?php svg('magic');?></span> will attempt to get the EXIF Information embedded in the Uploaded Image.
              </small>
            </div>
