...
<?php }else{?>
				<input type="text" id="tis" class="form-control textinput" data-tooltip data-original-title="<?php if($r['tis']==0){echo'Select a Date...';}else{echo date($config['dateFormat'],$r['tis']);}?>" value="<?php if($r['tis']!=0){echo date('Y-m-d h:m',$r['tis']);}?>" data-dbid="<?php echo$r['id'];?>" data-dbt="content" data-dbc="tis" placeholder="Select a Date...">
<?php }?>
			</div>
		</div>
		<div class="form-group">
			<label for="tie" class="control-label col-xs-5 col-sm-3 col-md-3 col-lg-2">Booking End</label>
			<div class="input-group col-xs-7 col-sm-9 col-md-9 col-lg-10">
<?php if($rs['contentType']=='events'){?>
				<input type="text" id="tie" class="form-control" value="<?php echo date($config['dateFormat'],$r['tie']);?>" readonly>
<?php }else{?>
				<input type="text" id="tie" class="form-control textinput" data-tooltip data-original-title="<?php if($r['tie']==''||$r['tie']==0){echo'Select a Date...';}else{echo date($config['dateFormat'],$r['tie']);}?>" value="<?php if($r['tie']!=0){echo date('Y-m-d h:m',$r['tie']);}?>" data-dbid="<?php echo$r['id'];?>" data-dbt="content" data-dbc="tie" placeholder="Select a Date...">
<?php }?>
			</div>
		</div>
		<div class="form-group">
			<label for="status" class="control-label col-xs-5 col-sm-3 col-md-3 col-lg-2">Status</label>
			<div class="input-group col-xs-7 col-sm-9 col-md-9 col-lg-10">
		...
		<div class="form-group">
			<label for="changeClient" class="control-label col-xs-3 col-lg-2">Choose Client</label>
...
<?php $sql=$db->query("SELECT id,contentType,code,title,assoc FROM content WHERE bookable='1' AND title!='' AND status='published' AND internal!='1' ORDER BY code ASC, title ASC");
	if($sql->rowCount()>0){?>
		<div class="form-group">
			<label for="rid" class="control-label col-xs-5 col-sm-3 col-md-3 col-lg-2">Booked</label>
			<div class="input-group col-xs-7 col-sm-9 col-md-9 col-lg-10">
					...
					<tr>
						...
						<td>
							<div id="controls_<?php echo$r['id'];?>" class="btn-group pull-right">
								<a class="btn btn-info btn-xs<?php if($r['status']=='delete')echo' hidden';?>" href="admin/bookings/edit/<?php echo$r['id'];?>"<?php if($config['options']{4}==1)echo' data-toggle="tooltip" title="Edit"';?>><i class="libre libre-edit visible-xs"></i><span class="hidden-xs">Edit</span></a>
<?php		if($user['rank']==1000||$user['options']{0}==1){?>
