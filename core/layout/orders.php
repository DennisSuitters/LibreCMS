...
<div class="panel panel-default">
  <div class="panel-heading clearfix">
    ...
    <div class="btn-group pull-right">
      <a class="btn btn-default" href="<?php echo URL.$settings['system']['admin'].'/orders';?>" data-toggle="tooltip" data-placement="left" title="Back"><?php svg('libre-gui-back');?></a>
      <button class="btn btn-default" onclick="$('#sp').load('core/email_order.php?id=<?php echo$r['id'];?>&act=print');" data-toggle="tooltip" data-placement="left" title="Print Order"><?php svg('libre-gui-print');?></button>
      <button class="btn btn-default" onclick="$('#sp').load('core/email_order.php?id=<?php echo$r['id'];?>&act=');" data-toggle="tooltip" data-placement="left" title="Email Order"><?php svg('libre-gui-email-send');?></button>
      <?php if($help['orders_edit_text']!='')echo'<a target="_blank" class="btn btn-default info" href="'.$help['orders_edit_text'].'" data-toggle="tooltip" data-placement="left" title="Help">'.svg2('libre-gui-help').'</a>';if($help['orders_edit_video']!='')echo'<a href="#" class="btn btn-default info" data-toggle="modal" data-frame="iframe" data-target="#videoModal" data-video="'.$help['orders_edit_video'].'" data-tooltip="tooltip" data-placement="left" title="Watch Video Help">'.svg2('libre-gui-video').'</a>';?>
    </div>
  </div>
          ...
          <div class="form-group form-group-xs">
            ...
            <div class="input-group col-xs-9">
              ...
              <div class="input-group-btn">
                <a class="btn btn-default" href="mailto:<?php echo$client['email'];?>"><?php svg('libre-gui-email-send');?></a>
              </div>
            </div>
          </div>
          <div class="form-group form-group-xs">
...
<?php if($r['status']!='archived'){?>
              <div class="input-group-btn">
                <button class="btn btn-default dropdown-toggle" data-toggle="dropdown"><?php svg('libre-gui-add');?></button>
...
<div class="panel panel-default">
  <div class="panel-heading clearfix">
    ...
    <div class="btn-group pull-right">
      <button class="btn btn-default add dropdown-toggle" data-toggle="dropdown" data-placement="right" data-tooltip="tooltip" data-placement="left" title="Add"><?php svg('libre-gui-plus');?></button>
      ...
      <a class="btn btn-default" href="<?php echo URL.$settings['system']['admin'].'/orders/settings';?>" data-toggle="tooltip" data-placement="left" title="Settings"><?php svg('libre-gui-settings');?></a>
      <?php if($help['orders_text']!='')echo'<a target="_blank" class="btn btn-default info" href="'.$help['orders_text'].'" data-toggle="tooltip" data-placement="left" title="Help">'.svg2('libre-gui-help').'</a>';if($help['orders_video']!='')echo'<a href="#" class="btn btn-default info" data-toggle="modal" data-frame="iframe" data-target="#videoModal" data-video="'.$help['orders_video'].'" data-tooltip="tooltip" data-placement="left" title="Watch Video Help">'.svg2('libre-gui-video').'</a>';?>
    </div>
  </div>
  <div class="panel-body">
    <div id="notifications"></div>
...
<?php while($r=$s->fetch(PDO::FETCH_ASSOC)){
  ...
  $c=$cs->fetch(PDO::FETCH_ASSOC);?>
          <tr id="l_<?php echo$r['id'];?>"<?php echo($ti>$r['due_ti'])||($r['status']=='overdue')?' class="danger text-danger"':'';?>>
            <td>
              <small><a href="<?php echo URL.$settings['system']['admin'].'/orders/edit/'.$r['id'];?>"><?php echo$r['aid']!=''?$r['aid'].'<br>':'';echo$r['qid'].$r['iid'];?></a></small>
              <small class="visible-xs hidden-sm hidden-md hidden-lg"><?php echo$c['username'];echo$c['name']!=''?' ['.$c['name'].']':'';echo$c['business']!=''?' -> '.$c['business']:'';?></small>
            </td>
            <td class="hidden-xs">
            ...
            <td id="controls_<?php echo$r['id'];?>">
              <div class="btn-group pull-right">
                <?php echo$r['qid']!=''&&$r['aid']==''?'<a class="btn btn-default'.($r['status']=='delete'?' hidden':'').'" href="'.URL.$settings['system']['admin'].'/orders/to_invoice/'.$r['id'].'" data-toggle="tooltip" title="Convert to Invoice...">'.svg2('libre-gui-order-quotetoinvoice').'</a>':'';
                echo$r['aid']==''?'<button class="btn btn-default'.($r['status']=='delete'?' hidden':'').'" onclick="update(\''.$r['id'].'\',\'orders\',\'status\',\'archived\')" data-toggle="tooltip" title="Archive">'.svg2('libre-gui-archive').'</button>':'';?>
                <button class="btn btn-default" onclick="$('#sp').load('core/email_order.php?id=<?php echo$r['id'];?>&act=print');" data-toggle="tooltip" title="Print Order"><?php svg('libre-gui-print');?></button>
                <?php echo$c['email']!=''?'<button class="btn btn-default" onclick="$(\'#sp\').load(\'core/email_order.php?id='.$r['id'].'&act=\');" data-toggle="tooltip" title="Email Order">'.svg2('libre-gui-email-send').'</button>':'';?>
                <a class="btn btn-default<?php echo$r['status']=='delete'?' hidden':'';?>" href="<?php echo URL.$settings['system']['admin'].'/orders/duplicate/'.$r['id'];?>" data-toggle="tooltip" title="Duplicate"'><?php svg('libre-gui-copy');?></a>
                <a class="btn btn-default<?php echo$r['status']=='delete'?' hidden':'';?>" href="<?php echo URL.$settings['system']['admin'].'/orders/edit/'.$r['id'];?>" data-toggle="tooltip" title="Edit"><?php svg('libre-gui-edit');?></a>
<?php if($user['rank']>399){?>
                <button class="btn btn-default<?php echo$r['status']!='delete'?' hidden':'';?>" onclick="updateButtons('<?php echo$r['id'];?>','orders','status','')" data-toggle="tooltip" title="Restore"><?php svg('libre-gui-email-reply');?></button>
                <button class="btn btn-default trash<?php echo$r['status']=='delete'?' hidden':'';?>" onclick="updateButtons('<?php echo$r['id'];?>','orders','status','delete')" data-toggle="tooltip" title="Delete"><?php svg('libre-gui-trash');?></button>
                <button class="btn btn-default trash<?php echo$r['status']!='delete'?' hidden':'';?>" onclick="purge('<?php echo$r['id'];?>','orders')" data-toggle="tooltip" title="Purge"><?php svg('libre-gui-purge');?></button>
<?php }?>
              </div>
            </td>
          </tr>
