...
<div class="panel panel-default">
  <div class="panel-heading clearfix">
    ...
    <div class="pull-right">
      <div class="btn-group">
        <a class="btn btn-default add" href="<?php echo URL.$settings['system']['admin'].'/pages/add';?>" data-toggle="tooltip" data-placement="left" title="Add"><?php svg('libre-gui-add',($config['iconsColor']==1?true:null));?></a>
      </div>
      <div class="btn-group">
        <a class="btn btn-default" href="<?php echo URL.$settings['system']['admin'].'/pages/settings';?>" data-toggle="tooltip" data-placement="left" title="Settings"><?php svg('libre-gui-settings',($config['iconsColor']==1?true:null));?></a>
      </div>
      <div class="btn-group">
        <a target="_blank" class="btn btn-default info" href="https://github.com/DiemenDesign/LibreCMS/wiki/Administration#pages" data-toggle="tooltip" data-placement="left" title="Help"><?php svg('libre-gui-help',($config['iconsColor']==1?true:null));?></a>
...
$sm=$db->prepare("SELECT id,title,contentType,views FROM menu WHERE mid!=0 AND mid=:mid ORDER BY ord ASC");
  ...
  while($rm=$sm->fetch(PDO::FETCH_ASSOC)){?>
              <small id="s_<?php echo$rm['id'];?>" class="sub help-block zebra text-right">
                ...
                <span id="controls_<?php echo$rm['id'];?>" class="text-right">
                  <button class="btn btn-default btn-xs trash" onclick="$('#views<?php echo$rm['id'];?>').text('0');update('<?php echo$rm['id'];?>','menu','views','0');"><?php svg('libre-gui-eraser',($config['iconsColor']==1?true:null));?> <span id="views<?php echo$rm['id'];?>"><?php echo$rm['views'];?></span></button>
                  <a class="btn btn-default btn-xs" href="<?php echo URL.$settings['system']['admin'].'/pages/edit/'.$rm['id'];?>" data-toggle="tooltip" title="Edit"><?php svg('libre-gui-edit',($config['iconsColor']==1?true:null));?></a>
<?php echo($rm['contentType']=='page'?'<button class="btn btn-default btn-xs trash" onclick="purge(\''.$rm['id'].'\',\'menu\')" data-toggle="tooltip" title="Delete">'.svg2('libre-gui-trash',($config['iconsColor']==1?true:null)).'</button>':'');?>
                </span>
              </small>
              <script>
...
<?php }?>
            ...
            <td class="text-center hidden-xs">
              <button class="btn btn-default trash" onclick="$('#views<?php echo$r['id'];?>').text('0');update('<?php echo$r['id'];?>','menu','views','0');"><?php svg('libre-gui-eraser',($config['iconsColor']==1?true:null));?> <span id="views<?php echo$r['id'];?>"><?php echo$r['views'];?></span></button>
            </td>
            <td class="text-center hidden-xs">
<?php echo($r['contentType']!='index'?'<div class="checkbox checkbox-success"><input type="checkbox" id="active'.$r['id'].'" data-dbid="'.$r['id'].'" data-dbt="menu" data-dbc="active" data-dbb="0"'.($r['active']==1?' checked':'').'><label for="active'.$r['id'].'"/></div>':'');?>
            ...
            <td id="controls_<?php echo$r['id'];?>" class="text-right">
              <a class="btn btn-default" href="<?php echo URL.$settings['system']['admin'].'/pages/edit/'.$r['id'];?>" data-toggle="tooltip" title="Edit"><?php svg('libre-gui-edit',($config['iconsColor']==1?true:null));?></a>
<?php echo($r['contentType']=='page'?'<button class="btn btn-default trash" onclick="purge(\''.$r['id'].'\',\'menu\')" data-toggle="tooltip" title="Delete">'.svg2('libre-gui-trash',($config['iconsColor']==1?true:null)).'</button>':'');?>
            </td>
          </tr>
<?php }?>
...
<?php while($ro=$so->fetch(PDO::FETCH_ASSOC))echo'<li><a href="'.URL.$settings['system']['admin'].'/pages/edit/'.$ro['id'].'">'.$ro['title'].'</a></li>';?>
    ...
    <div class="pull-right">
      ...
      <div class="btn-group">
        <a target="_blank" class="btn btn-default info" href="https://github.com/DiemenDesign/LibreCMS/wiki/Administration#pages-edit-content" data-toggle="tooltip" data-placement="left" title="Help"><?php svg('libre-gui-help',($config['iconsColor']==1?true:null));?></a>
        <span data-toggle="tooltip" data-placement="left" title="Watch Video Help"><a href="#" class="btn btn-default info" data-toggle="modal" data-frame="iframe" data-target="#videoModal" data-video="https://www.youtube.com/embed/FsXG1YSqcjU"><?php svg('libre-gui-video',($config['iconsColor']==1?true:null));?></a></span>
      </div>
    </div>
  </div>
...
<?php echo($user['rank']>899?'<div class="input-group-btn"><button class="btn btn-default fingerprint hidden-xs" data-toggle="popover" data-dbgid="title">'.svg2('libre-gui-fingerprint',($config['iconsColor']==1?true:null)).'</button>
</div>':'');
if($r['suggestions']==1){
  $ss=$db->prepare("SELECT rid FROM suggestions WHERE rid=:rid AND t=:t AND c=:c");
  $ss->execute(array(':rid'=>$r['id'],':t'=>'menu',':c'=>'title'));
...
<?php echo($user['rank']>899?'<button class="btn btn-default fingerprint hidden-xs" data-toggle="popover" data-dbgid="notesda">'.svg2('libre-gui-fingerprint',($config['iconsColor']==1?true:null)).'</button>':'');
if($r['suggestions']==1){
  $ss=$db->prepare("SELECT rid FROM suggestions WHERE rid=:rid AND t=:t AND c=:c");
  $ss->execute(array(':rid'=>$r['id'],':t'=>'menu',':c'=>'notes'));
      ...
      <div role="tabpanel" class="tab-pane" id="page-images">
        <fieldset class="control-fieldset">
          ...
          <div class="form-group">
            ...
            <div class="input-group col-xs-7 col-sm-9 col-lg-10">
<?php echo($user['rank']>899?'<div class="input-group-btn"><button class="btn btn-default fingerprint hidden-xs" data-toggle="popover" data-dbgid="coverURL">'.svg2('libre-gui-fingerprint',($config['iconsColor']==1?true:null)).'</button></div>':'');?>
              ...
              <div class="input-group-btn">
                <button class="btn btn-default trash" onclick="coverUpdate('<?php echo$r['id'];?>','menu','coverURL','');"><?php svg('libre-gui-trash',($config['iconsColor']==1?true:null));?></button>
              </div>
            </div>
          ...
          <div class="form-group clearfix">
            ...
            <div class="input-group col-xs-7 col-sm-9 col-lg-10">
<?php echo($user['rank']>899?'<div class="input-group-btn"><button class="btn btn-default fingerprint hidden-xs" data-toggle="popover" data-dbgid="cover">'.svg2('libre-gui-fingerprint',($config['iconsColor']==1?true:null)).'</button></div>':'');?>
              ...
              <div class="input-group-btn">
                <button class="btn btn-default" onclick="elfinderDialog('<?php echo$r['id'];?>','menu','cover');"><?php svg('libre-gui-browse-media',($config['iconsColor']==1?true:null));?></button>
              </div>
...
elseif($r['coverURL'] != '')
  ...
  echo'<img id="coverimage" src="'.NOIMAGE.'">';?>
              ...
              <div class="input-group-btn">
                <button class="btn btn-default trash" onclick="coverUpdate('<?php echo$r['id'];?>','menu','cover','');"><?php svg('libre-gui-trash',($config['iconsColor']==1?true:null));?></button>
              </div>
            </div>
...
<?php echo($user['rank']>899?'<div class="input-group-btn"><button class="btn btn-default fingerprint hidden-xs" data-toggle="popover" data-dbgid="coverVideo">'.svg2('libre-gui-fingerprint',($config['iconsColor']==1?true:null)).'</button></div>':'');?>
        ...
        <fieldset class="control-fieldset">
          ...
          <div class="form-group">
            ...
            <div class="input-group col-xs-7 col-sm-9 col-lg-10">
<?php echo($user['rank']>899?'<div class="input-group-btn"><button class="btn btn-default fingerprint hidden-xs" data-toggle="popover" data-dbgid="attributionImageTitle">'.svg2('libre-gui-fingerprint',($config['iconsColor']==1?true:null)).'</button></div>':'');?>
          ...
          <div class="form-group">
            ...
            <div class="input-group col-xs-7 col-sm-9 col-lg-10">
<?php echo($user['rank']>899?'<div class="input-group-btn"><button class="btn btn-default fingerprint hidden-xs" data-toggle="popover" data-dbgid="attributionImageName">'.svg2('libre-gui-fingerprint',($config['iconsColor']==1?true:null)).'</button></div>':'');?>
...
while($rs=$s->fetch(PDO::FETCH_ASSOC))echo'<option value="'.$rs['name'].'"/>';?>
          ...
          <div class="form-group">
            ...
            <div class="input-group col-xs-7 col-sm-9 col-lg-10">
<?php echo($user['rank']>899?'<div class="input-group-btn"><button class="btn btn-default fingerprint hidden-xs" data-toggle="popover" data-dbgid="attributionImageURL">'.svg2('libre-gui-fingerprint',($config['iconsColor']==1?true:null)).'</button></div>':'');?>
        ...
        <form target="sp" method="post" enctype="multipart/form-data" action="core/add_data.php">
          ...
          <div class="form-group">
            <div class="input-group">
              ...
              <div class="input-group-btn">
                <button class="btn btn-default" onclick="elfinderDialog('<?php echo$r['id'];?>','media','file');return false;"><?php svg('libre-gui-browse-media',($config['iconsColor']==1?true:null));?></button>
              </div>
              <div class="input-group-btn">
  ...
  list($width,$height)=getimagesize($rm['file']);?>
          <li id="media_items_<?php echo$rm['id'];?>" class="col-xs-6 col-sm-3">
            <div class="panel panel-default media">
              ...
              <div class="panel-body">
                <a href="<?php echo $rm['file'];?>" data-srcset="<?php echo$rm['file'];?> <?php echo$width;?>w" data-fancybox="gallery" data-width="<?php echo$width;?>" data-height="<?php echo$height;?>" data-caption="<?php echo $rm['title'];if($rm['seoCaption'])echo' - '.$rm['seoCaption'];?>">
...
<?php }?>
      ...
      <div role="tabpanel" class="tab-pane" id="page-seo">
        <div class="form-group">
          ...
          <div class="input-group col-xs-7 col-sm-9 col-lg-10">
            <div class="input-group-btn">
              <button class="btn btn-default analytics hidden-xs" data-toggle="analytics" data-id="<?php echo$r['id'];?>" data-t="menu" data-u="<?php echo URL.$r['contentType'];?>" data-analytics="social"><?php svg('libre-seo-social',($config['iconsColor']==1?true:null));?> Social</button>
              <button class="btn btn-default analytics hidden-xs" data-toggle="analytics" data-id="<?php echo$r['id'];?>" data-t="menu" data-u="<?php echo URL.$r['contentType'];?>" data-analytics="google"><?php svg('libre-seo-google',($config['iconsColor']==1?true:null));?> Google</button>
              <button class="btn btn-default analytics hidden-xs" data-toggle="analytics" data-id="<?php echo$r['id'];?>" data-t="menu" data-u="<?php echo URL.$r['contentType'];?>" data-analytics="alexa"><?php svg('libre-seo-alexa',($config['iconsColor']==1?true:null));?> Alexa</button>
              <button class="btn btn-default analytics hidden-xs" data-toggle="analytics" data-id="<?php echo$r['id'];?>" data-t="menu" data-u="<?php echo URL.$r['contentType'];?>" data-analytics="moz"><?php svg('libre-seo-moz',($config['iconsColor']==1?true:null));?> Moz</button>
            </div>
          </div>
        </div>
        <script>
        ...
        <div class="form-group">
          ...
          <div class="input-group col-xs-7 col-sm-9 col-lg-10">
<?php echo($user['rank']>899?'<div class="input-group-btn"><button class="btn btn-default fingerprint hidden-xs" data-toggle="popover" data-dbgid="views">'.svg2('libre-gui-fingerprint',($config['iconsColor']==1?true:null)).'</button></div>':'');?>
            ...
            <div class="input-group-btn">
              <button class="btn btn-default trash" onclick="$('#views').val('0');update('<?php echo$r['id'];?>','menu','views','0');"><?php svg('libre-gui-eraser',($config['iconsColor']==1?true:null));?></button>
            </div>
          </div>
        </div>
        <div class="form-group clearfix">
          ...
          <div class="input-group col-xs-7 col-sm-9 col-lg-10">
<?php echo($user['rank']>899?'<div class="input-group-btn"><button class="btn btn-default fingerprint hidden-xs" data-toggle="popover" data-dbgid="metaRobots">'.svg2('libre-gui-fingerprint',($config['iconsColor']==1?true:null)).'</button></div>':'');
if($r['suggestions']==1){
  $ss=$db->prepare("SELECT rid FROM suggestions WHERE rid=:rid AND t=:t AND c=:c");
  $ss->execute(array(':rid'=>$r['id'],':t'=>'menu',':c'=>'metaRobots'));
  echo($ss->rowCount()>0?'<div class="input-group-btn" data-toggle="tooltip" data-placement="top" title="Suggestions"><button class="btn btn-default suggestion hidden-xs" data-toggle="popover" data-dbgid="metaRobots">'.svg2('libre-gui-lightbulb',($config['iconsColor']==1?true:null),'','green').'</button></div>':'');
}?>
...
<?php echo($user['rank']>899?'<div class="input-group-btn" data-toggle="tooltip" data-placement="top" title="Add Suggestion"><button class="btn btn-default addsuggestion hidden-xs" data-toggle="popover" data-dbgid="metaRobots">'.svg2('libre-gui-idea',($config['iconsColor']==1?true:null)).'</button></div>':'');?>
        ...
        <div class="form-group clearfix">
          ...
          <div class="input-group col-xs-7 col-sm-9 col-lg-10">
<?php echo($user['rank']>899?'<div class="input-group-btn"><button class="btn btn-default fingerprint hidden-xs" data-toggle="popover" data-dbgid="seoTitle">'.svg2('libre-gui-fingerprint',($config['iconsColor']==1?true:null)).'</button></div>':'');
...
<?php if($r['suggestions']==1){
  $ss=$db->prepare("SELECT rid FROM suggestions WHERE rid=:rid AND t=:t AND c=:c");
  $ss->execute(array(':rid'=>$r['id'],':t'=>'menu',':c'=>'seoTitle'));
...
<?php echo($user['rank']>899?'<div class="input-group-btn" data-toggle="tooltip" data-placement="top" title="Add Suggestion"><button class="btn btn-default addsuggestion hidden-xs" data-toggle="popover" data-dbgid="seoTitle">'.svg2('libre-gui-idea',($config['iconsColor']==1?true:null)).'</button></div>':'');?>
        ...
        <div class="form-group clearfix">
          ...
          <div class="input-group col-xs-7 col-sm-9 col-lg-10">
<?php echo($user['rank']>899?'<div class="input-group-btn"><button class="btn btn-default fingerprint hidden-xs" data-toggle="popover" data-dbgid="seoCaption">'.svg2('libre-gui-fingerprint',($config['iconsColor']==1?true:null)).'</button></div>':'');
...
<?php if($r['suggestions']==1){
  $ss=$db->prepare("SELECT rid FROM suggestions WHERE rid=:rid AND t=:t AND c=:c");
  $ss->execute(array(':rid'=>$r['id'],':t'=>'menu',':c'=>'seoCaption'));
...
<?php echo($user['rank']>899?'<div class="input-group-btn" data-toggle="tooltip" data-placement="top" title="Add Suggestion"><button class="btn btn-default addsuggestion hidden-xs" data-toggle="popover" data-dbgid="seoCaption">'.svg2('libre-gui-idea',($config['iconsColor']==1?true:null)).'</button></div>':'');?>
      ...
      <div class="form-group clearfix">
        ...
        <div class="input-group col-xs-7 col-sm-9 col-lg-10">
<?php echo($user['rank']>899?'<div class="input-group-btn"><button class="btn btn-default fingerprint hidden-xs" data-toggle="popover" data-dbgid="seoDescription">'.svg2('libre-gui-fingerprint',($config['iconsColor']==1?true:null)).'</button></div>':'');
...
<?php if($r['suggestions']==1){
  $ss=$db->prepare("SELECT rid FROM suggestions WHERE rid=:rid AND t=:t AND c=:c");
  $ss->execute(array(':rid'=>$r['id'],':t'=>'menu',':c'=>'seoDescription'));
...
<?php echo($user['rank']>899?'<div class="input-group-btn" data-toggle="tooltip" data-placement="top" title="Add Suggestion"><button class="btn btn-default addsuggestion hidden-xs" data-toggle="popover" data-dbgid="seoDescription">'.svg2('libre-gui-idea',($config['iconsColor']==1?true:null)).'</button></div>':'');?>
      ...
      <div class="form-group">
        ...
        <div class="input-group col-xs-7 col-sm-9 col-lg-10">
<?php echo($user['rank']>899?'<div class="input-group-btn"><button class="btn btn-default fingerprint hidden-xs" data-toggle="popover" data-dbgid="seoKeywords">'.svg2('libre-gui-fingerprint',($config['iconsColor']==1?true:null)).'</button></div>':'');
if($r['suggestions']==1){
  $ss=$db->prepare("SELECT rid FROM suggestions WHERE rid=:rid AND t=:t AND c=:c");
  $ss->execute(array(':rid'=>$r['id'],':t'=>'menu',':c'=>'seoKewords'));
...
<?php }?>
      <div class="form-group">
        ...
        <div class="input-group col-xs-7 col-sm-9 col-lg-10">
<?php echo($user['rank']>899?'<div class="input-group-btn"><button class="btn btn-default fingerprint hidden-xs" data-toggle="popover" data-dbgid="menu">'.svg2('libre-gui-fingerprint',($config['iconsColor']==1?true:null)).'</button></div>':'');?>
