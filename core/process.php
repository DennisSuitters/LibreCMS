...
if($view=='page'){
  ...
  $sp->execute(array(':contentType'=>$view,':title'=>str_replace('-',' ',$args[0])));
}else{
...
foreach($tag as$tag1){
  ...
  if($include!=''){
    ...
    if(file_exists(THEME.$amp.DS.$include.'.html'))
      ...
      $html='';
    if(file_exists('core'.DS.'layout'.DS.$include.'.php'))
      require'core'.DS.'layout'.DS.$include.'.php';
    else
...
$head=str_replace(array(
  ...
  (empty($seoTitle)?ucfirst($view).' - '.$config['seoTitle']:$page['seoTitle'].' - '.$config['seoTitle']),
  ...
  '<script>/*<![CDATA[*/'.htmlspecialchars_decode($config['ga_tracking'],ENT_QUOTES).'/*]]>*/</script>'
),$head);
if((isset($_GET['theme'])&&file_exists('layout'.DS.$_GET['theme']))||(isset($_GET['amp'])&&$_GET['amp']=='amped')){
  $doc=new DOMDocument;
  ...
  $content=preg_replace('/^<!DOCTYPE.+?>/','',str_replace(array('<html>','</html>','<body>','</body>'),array('','','',''),$doc->saveHTML())).'</body></html>';
	if(isset($_GET['amp'])&&$_GET['amp']=='amped')$content=preg_replace('/(<[^>]*) style=("[^"]+"|\'[^\']+\')([^>]*>)/i', '$1$3',$content);
}
if(isset($_SESSION['rank'])&&$_SESSION['rank']>899&&$config['development']==1)$content.='<div style="text-align:right;padding:10px;">Memory Used: '.size_format(memory_get_usage()).' | Process Time: '.elapsed_time().'</div>';
