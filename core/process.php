...
foreach($tag as$tag1){
  ...
  if($include!=''){
    ...
    if(file_exists(THEME.'view'.DS.$include.'.php')) require THEME.'view'.DS.$include.'.php';
    elseif(file_exists('core'.DS.'view'.DS.$include.'.php')) require'core'.DS.'view'.DS.$include.'.php';
    else require'core'.DS.'view'.DS.'content.php';
    $req=$include;
  }
  if($inbed!=''){
    ...
    if(file_exists(THEME.'view'.DS.$inbed.'.php')) require THEME.'view'.DS.$inbed.'.php';
    elseif(file_exists('core'.DS.'view'.DS.$inbed.'.php')) require'core'.DS.'view'.DS.$inbed.'.php';
    else require'core'.DS.'view'.DS.'content.php';
    $req=$inbed;
  }
}
...
if(isset($args[1])&&$args[1]!=''&&isset($r['seoKeywords']))$seoKeywords=$r['seoKeywords'];
...
if(isset($_SESSION['rank'])&&$_SESSION['rank']>899&&$config['development']==1)$content.='<div style="text-align:right;padding:10px;">Page Views: '.$page['views'].' | Memory Used: '.size_format(memory_get_usage()).' | Process Time: '.elapsed_time().'</div>';
