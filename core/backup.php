...
if($compression)
    $file=$nowtimename.'.sql.gz';$zp=gzopen('../media/backup/'.$file,"a9");
else
    $file=$nowtimename.'.sql';$handle=fopen('../media/backup/'.$file,'a+');
...
if(file_exists('../media/backup/'.$file)){
    ...
    window.top.window.$('#backup').append('<div id="l_<?php echo$fileid;?>" class="form-group"><label class="control-label col-xs-5 col-sm-3 col-lg-2">&nbsp;</label><div class="input-group col-xs-7 col-sm-9 col-lg-10"><a class="btn btn-default btn-block" href="media/backup/<?php echo$file;?>">Click to Download <?php echo$file;?></a><div class="input-group-btn"><button class="btn btn-default trash" onclick="removeMedia(\'<?php echo$file;?>\')"><?php svg('trash');?></button></div></div></div>');
