...
if($t!=''){
  ...
  $id=$db->lastInsertId();?>
<script>
  window.top.window.$('#postoption').append('<div id="l_<?php echo$id;?>" class="form-group row"><div class="input-group col"><span class="input-group-text"><?php echo localize('Service');?></span><input type="text" class="form-control" name="service" value="<?php echo$t;?>" readonly role="textbox"><span class="input-group-text"><?php echo localize('Cost');?></span><input type="text" class="form-control" name="cost" value="<?php echo$v;?>" readonly role="textbox"><div class="input-group-append"><form target="sp" action="core/purge.php" role="form"><input type="hidden" name="id" value="<?php echo$id;?>" role="textbox"><input type="hidden" name="t" value="choices" role="textbox"><button class="btn btn-secondary trash" data-tooltip="tooltip" title="<?php echo localize('Delete');?>" role="button" aria-label="<?php echo localize('aria_delete');?>"><?php svg('libre-gui-trash');?></button></form></div></div></div>');
</script>
<?php }
