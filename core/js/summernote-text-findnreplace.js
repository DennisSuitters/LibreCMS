    ...
    'findnreplace':function(context){
              ...
              if(toolButton.hasClass('btn-success')){
                ...
                $('[data-toggle="findnreplace"]').popover({
                  ...
                  content:'<div class="form-group">'+
                    '<label for="note-findnreplace-find" class="control-label col-xs-3">'+lang.findnreplace.find+'</label>'+
                      '<div class="input-group col-xs-9">'+
                        '<input id="note-findnreplace-find" type="text" class="note-findnreplace-find form-control" value="">'+
                        '<div class="input-group-btn">'+
                          '<button class="note-findnreplace-find-btn btn btn-default">'+lang.findnreplace.findBtn+'</button>'+
                        '</div>'+
                      '</div>'+
                    '</div>'+
                    '<div class="form-group">'+
                      '<label for="note-findnreplace-replace" class="control-label col-xs-3">'+lang.findnreplace.replace+'</label>'+
                      '<div class="input-group col-xs-9">'+
                        '<input id="note-findnreplace-replace" type="text" class="note-findnreplace-replace form-control" value="">'+
                        '<div class="input-group-btn">'+
                          '<button class="note-findnreplace-replace-btn btn btn-default">'+lang.findnreplace.replaceBtn+'</button>'+
                        '</div>'+
                      '</div>'+
                    '</div>'
                }).popover(showPopover);
      ...
      this.bindLabels=function(){
        ...
        $findBtn.click(function(e){
          ...
          if(findText!=''){
            if(replaceText==''){
              var replacedText=text.replace("gi",'<u class="note-findnreplace" style="'+options.findnreplace.highlight+'">'+findText+'</u>');
            }else{
