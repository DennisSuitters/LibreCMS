...
class Zebra_Image {
      ...
      if ($this->_create_from_source()) {
        ...
        if (is_array($filter)) {
          ...
          if (!@call_user_func_array('imagefilter', array_merge(array($target_identifier, constant('IMG_FILTER_' . strtoupper($filter))), array_slice($arguments, 1))))
            trigger_error('Invalid arguments used for "' . strtoupper($arguments[0]) . '" filter', E_USER_WARNING);
        } else trigger_error('Filter "' . strtoupper($arguments[0]) . '" is not available', E_USER_WARNING); return $this -> _write_image($target_identifier);
      }
    return false;
  }
  ...
  private function _write_image ($identifier) {
    ...
    if ($disabled_functions == '' || strpos('chmod', $disabled_functions) === false) chmod($this->target_path, intval($this->chmod_value, 8));else $this->error = 8;
    if ($this->preserve_time && isset($this->source_image_time)) @touch($this -> target_path, $this->source_image_time);
    return true;
  }
}